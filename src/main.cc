...
TestWindow::TestWindow(CompanyInfo &company, string window) : company_(company), windowName_(window), windowComponent_(company.info_->slidingWindowPairs_.at(window)), tableStartRow_(company.tableStartRow_)
{
    ...
    {
        interval_.emplace_back(company_.testStartRow_);
        interval_.emplace_back(company_.testEndRow_);
    }
...
void TestWindow::find_W_test()
{
    ...
    startRow.emplace_back(company_.testStartRow_);
    ...
    {
        endRow.emplace_back(company_.testEndRow_);
    }
    check_startRowSize_endRowSize((int)startRow.size(), (int)endRow.size());
    interval_ = save_startRow_EndRow(startRow, endRow);
}
...
void TrainWindow::find_regular_M_train(vector<int> &endRow, vector<int> &startRow)
{
    for (int intervalIndex = 0; intervalIndex < intervalSize_; intervalIndex += 2)
    {
        ...
        endRow.emplace_back(TestWindow::interval_[intervalIndex] - 1 + tableStartRow_);
    }
}
...
void TrainWindow::find_D_train()
{
    ...
    for (int intervalIndex = 0; intervalIndex < intervalSize_; intervalIndex += 2)
    {
        startRow.emplace_back(TestWindow::interval_[intervalIndex] - trainLength_ + tableStartRow_);
        endRow.emplace_back(TestWindow::interval_[intervalIndex] - 1 + tableStartRow_);
    }
    check_startRowSize_endRowSize((int)startRow.size(), (int)endRow.size());
    interval_ = save_startRow_EndRow(startRow, endRow);
...
{ // 裝RSI資料
public:
    static const inline vector<int> eachVariableBitsNum_ = {4, 7, 7}; // RSI每個參數要多少個bits
...
void Particle::init(CompanyInfo *company, bool isRecordOn, vector<int> variables)
{
    ...
    else if (company_->info_->mixedTech_ && company_->info_->mixType_ == 2)
    { // 如果是mixType 2就要分別設定買賣的參數數量
...
void Particle::push_holdData_column_Name(bool hold, string &holdData, string *&holdDataPtr, string windowName)
{
    if (hold)
    {
        ...
        for (int i = 0; i < 20; i++)
        { // 要加上多的delimiter，pd.read_csv(file, index_col=0)才不會error，因為header要比column多
            holdData += ",";
        }
        holdData += "\n";
        // if (!company_->info_->mixedTech_) {
...
void TrainAPeriod::update_best(int renewBest)
{
    ...
    switch (renewBest)
    { // 0: 全域最佳解看binary更新，1: 全域最佳解看RoR更新
        case 0:
        {
...
public:
    ...
    {
        unique_lock<mutex> lock(mtx);
        cv.wait(lock, [&count_ = this->count_]()
                { return count_ > 0; });
        count_--;
    }
...
MixedTechChooseTrainFile::MixedTechChooseTrainFile(CompanyInfo *company, TrainWindow *window, vector<string> &techTrainFilePath) : company_(company), window_(window)
{
    ...
    // for (auto &techIndex : company_->info_->techIndexs_) {
    //     trainFilePaths.emplace_back(techTrainFilePath[techIndex]);
    // }
    // if (company_->info_->mixType_ == 3) {  // 如果是mixType 3，需要把mix的路徑放進去
    //     trainFilePaths.emplace_back(techTrainFilePath[5]);
    //     trainFilePaths.emplace_back(techTrainFilePath[6]);
    // }

    // 表現最好的組合
    if (company_->info_->mixType_ == 3)
    { // 如果是mixType 3，需要把mix的路徑放進去
        trainFilePaths.emplace_back(techTrainFilePath[6]);
        trainFilePaths.emplace_back(techTrainFilePath[3]);
        trainFilePaths.emplace_back(techTrainFilePath[0]);
        trainFilePaths.emplace_back(techTrainFilePath[5]);
    }
    ...
    {
        diffTechTrainFilePath.emplace_back(get_path(trainPath + window->windowName_));
    }
...
{ // 在不同指標訓練期中選比較好的出來
    ...
    {
        everyRoR.emplace_back(stod(trainFile[10][1]));
    }
...
{ // mixType_ 1用，應該不會再用到
    ...
    tmps.pop_back();
    mixedStrategies_.emplace_back(tmps);
}
...
{ // 讀tech file
    ...
    {
        tmpTechIndexes.emplace_back(additionTech);
    }
    ...
    for (auto i : tmpTechIndexes)
    {
        ...
        {
            threads.emplace_back(thread(get_table, ref(tables_), i, ref(sem)));
...
void Train::train_a_company()
{
    ...
    for (auto windowName : company_->info_->slidingWindows_)
    {
        ...
        {
            windowThreads.emplace_back(thread(&Train::train_a_window, this, window));
        }
        else
        {
...
TrainLoop::TrainLoop(CompanyInfo &company) : company_(company), tables_({TechTable(&company, company.info_->techIndex_)}), sem_(company.info_->companyThreadNum_)
{
    ...
    {
        loopThread.emplace_back(thread([&]()
                                    { Train(company_, tables_, sem_); }));
    ...
    { // 如果mixType 2，要把兩種指標傳統策略混合
        for (auto buyStrategy : allTraditionStrategy_[company_->info_->techIndexs_[0]])
        {
            for (auto sellStrategy : allTraditionStrategy_[company_->info_->techIndexs_[1]])
            {
                ...
                traditionStrategy_.emplace_back(tmpStrategy);
            }
        }
    }
...
CalARR::CalOneCompanyARR::CalOneCompanyARR(CompanyInfo &company, vector<CompanyWindowARRContainer> &allCompanyWindowsARR, vector<Rank> &allCompanyWindowRank, int trainOrTestIndex) : company_(company), allCompanyWindowsARR_(allCompanyWindowsARR), allCompanyWindowRank_(allCompanyWindowRank), trainOrTestIndex_(trainOrTestIndex)
{
    ...
    thisCompanyWindowARR_.windowsARR_.emplace_back(cal_BH_ARR());
    rank_algo_and_tradition_window(thisCompanyWindowARR_.windowsARR_);
    allCompanyWindowsARR_.emplace_back(thisCompanyWindowARR_);
}
...
void CalARR::CalOneCompanyARR::cal_windows_ARR(TrainWindow &window)
{
    ...
    thisCompanyWindowARR_.windowsARR_.emplace_back(tmpWinodwARR_);
}
...
double CalARR::CalOneCompanyARR::cal_one_ARR(string &RoRoutData, TrainWindow &window, string &stratgyFilePath, bool tradition)
{
    ...
    double totalRoR = 0;
    if (!exists(stratgyFilePath + window.windowName_))
    { // 如果傳統的還沒做，回傳-1
        return -1;
    }
...
void CalARR::CalOneCompanyARR::rank_algo_and_tradition_window(vector<WindowARR> &windowsARR)
{
    ...
    {
        tmpRank.traditionWindowRank_.emplace_back(windowARR.rank_);
    }
    ...
    for (auto &windowARR : windowsARR)
    {
        tmpRank.algoWindowRank_.emplace_back(windowARR.rank_);
    }
    sort_by_algo_ARR(windowsARR);
    allCompanyWindowRank_.emplace_back(tmpRank);
}
...
class MergeARRFile
{ // 這邊會將mixed ARR file全部放在一起輸出成一個csv方便比較，可能用不到，直接打開csv手動複製就好
public:
    Info *info_;
...
MergeARRFile::MergeARRFile(Info *info) : info_(info)
{
    if (info_->techIndexs_.size() > 1)
    {
        ...
        {
            files.emplace_back(read_data(info_->rootFolder_ + info_->allTech_[techIndex] + "_test_ARR.csv"));
        }
        ...
        for (auto &file : files)
        {
            for (size_t row = 0; row < file.size(); row++)
            {
                ...
                {
                    newFile[row].emplace_back(file[row][col]);
                }
            }
        }
...
class SortARRFileBy
{ // 選擇特定的ARR file，將整份文件以視窗名稱排序，或是以ARR排序(用第幾個col調整)
public:
...
SortARRFileBy::equalIterType SortARRFileBy::findEqualSign(vector<vector<string>> &inputFile)
{
    ...
    for (auto iter = inputFile.begin(); iter != inputFile.end(); iter++)
    {
        if ((*iter).front().front() == '=')
        {
            equalSignIter.emplace_back(iter);
        }
    }
    equalSignIter.emplace_back(inputFile.end());
...
void SortARRFileBy::sortByName(vector<vector<string>> &inputFile, equalIterType &equalSignIter)
{
    ...
    {
        sort(equalSignIter[eachEqualIndex] + 1, equalSignIter[eachEqualIndex + 1], [](const vector<string> &s1, const vector<string> &s2) { return s1[0] < s2[0]; });
    }
}
...
FindBestHold::FindBestHold(Info *info, string ARRFileName, string algoOrTrad) : info_(info), trainOrTest_(cut_string(ARRFileName, '_')[0]), algoOrTrad_(algoOrTrad)
{
    ...
    {
        (*bestIter1).emplace_back((*bestIter2)[1]);
    }
...
int ResetFile::reset_file(path fileName)
{
    ...
    //         stratey.emplace_back(stoi(file[i + 12][1]));
    //     }
    //     vector<string> s;
    //     for (auto i : stratey) {
...
public:
    RunMode(Info &info, vector<path> &companyPricePaths) : info_(info), companyPricePaths_(companyPricePaths), sem_(info.companyThreadNum_)
    {
        ...
        {
            companiesInfo.emplace_back(CompanyInfo(companyPricePath, _info));
        }
        for (auto &company : companiesInfo)
        {
            companyThread.emplace_back(thread(&RunMode::run_mode, this, ref(company)));
            this_thread::sleep_for(0.1s);
        }
...
static vector<path> set_company_price_paths(const Info &info)
{
    ...
    if (info.setCompany_ != "all")
    {
        ...
        {
            return distance(companiesPricePath.begin(), 
                find_if(companiesPricePath.begin(), companiesPricePath.end(), [&](path &pricePath) 
                { 
                    return pricePath.stem() == traget; 
                }));
        };
        if (info.setCompany_.find(",") != string::npos)
        {
            ...
            {
                tmp.emplace_back(companiesPricePath[find_Index(companyName)]);
            }
