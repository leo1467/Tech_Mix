...
public:
    ...
    vector<MixedStrategy> mixedStrategy_;
    ...
    MixedTechChooseTrainFile(CompanyInfo *company, TrainWindow *window, vector<string> &techTrainFilePath) : company_(company), window_(window), goodTrainFiles(window->intervalSize_ / 2) {
            ...
            switch (company_->info_->mixType_) {
                ...
                case 1: {
                    for (auto iter = aPeriodTrainFiles.begin(); iter != aPeriodTrainFiles.end(); iter++) {
                        MixedStrategy tmp;
                        tmp.buy_.techIndex_ = find_index_of_string_in_vec(company_->info_->allTech_, (*iter)[0][1]);
                        int decimalNum;
                        ...
                        tmp.sell_ = tmp.buy_;
                        mixedStrategy_.push_back(tmp);
                    }
                    vector<MixedStrategy> tmps;
                    for (auto iIter = mixedStrategy_.begin(); iIter != mixedStrategy_.end(); iIter++) {
                        for (auto jIter = mixedStrategy_.begin(); jIter < mixedStrategy_.end(); jIter++) {
                            MixedStrategy tmp;
                            tmp.buy_ = (*iIter).buy_;
